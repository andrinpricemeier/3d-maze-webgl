 ...
 */
  ...
  */
export function createFaceIterator(rawObject, logger) {
    if (!logger)
        logger = console;
    ...
    for(let i = 0; i < lines.length; i++) {
        ...
        if(line.indexOf('v') === 0) {
            line.replace('v', '')
                ...
                .split(' ')
                .map(f => Number(f))
                .forEach(v => vertices.push(v));

        } else if(line.indexOf('f') === 0) {
            line.replace('f', '')
                .trim().split(' ')
                .map(i => Math.trunc(i))
                .forEach(i => faces.push(i));
        }
    }
...
FaceIterator.prototype.next = function() {
    ...
    const result = {
        v1: vec3.fromValues(this.getVertex(indices[0])),
        v2: vec3.fromValues(this.getVertex(indices[1])),
        v3: vec3.fromValues(this.getVertex(indices[2])),
        indices: {
            v1: indices[0],
            v2: indices[1],
            v3: indices[2]
